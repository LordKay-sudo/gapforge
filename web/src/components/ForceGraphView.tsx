...
/** Fixed ring layout so every node stays in frame for small 1-hop subgraphs. */
function layoutRing(nodeList: GraphNode[], centerId: string): GraphNode[] {
  ...
  const radius = Math.max(75, 32 * others.length);
    ...
    <div ref={containerRef} className="graph-panel">
      <ForceGraph2D
        ...
        nodeCanvasObject={(node, ctx, globalScale) => {
          ...
          ctx.fillText(label, n.x ?? 0, (n.y ?? 0) + r + 4);
        }}
      />
    </div>
  );
