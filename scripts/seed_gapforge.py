...
def seed_case(session, data: dict) -> None:
    drug = data["drug"]
...
def seed_gapforge(driver) -> None:
    ...
    if not case_paths:
        raise FileNotFoundError(f"No GapForge case JSON files in {CASES_DIR}")

    with driver.session() as session:
        run_constraints(session)
        for path in case_paths:
            data = json.loads(path.read_text(encoding="utf-8"))
            seed_case(session, data)
            print(f"  seeded case: {path.name} ({data['case_study']['id']})")
        ...
        print(
            ...
            f"from {len(case_paths)} case file(s)"
        )
